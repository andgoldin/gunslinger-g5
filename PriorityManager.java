...
public class PriorityManager {
	...
	private static final int
			...
			ENEMY_SHOOTS_ME = 5,
			NEUTRAL_SHOOTS_ME = 3,
			ENEMY_SHOOTS_ME_LOTS = 5,
			NEUTRAL_SHOOTS_ME_LOTS = 5,
			ENEMY_SHOOTS_NONFRIEND = 5,
			NONENEMY_SHOOTS_ENEMY = 3,
			ENEMY_SHOOTS_FRIEND = 3,
			NEUTRAL_SHOOTS_FRIEND = 3,
			ENEMY_SHOOTS_FRIEND_LOTS = 3,
			NEUTRAL_SHOOTS_FRIEND_LOTS = 1;
				...
				if (shooters != null) {
					for (int j = 0; j < shooters.length; j++) {
						if (alive[shooters[j]]) {
							...
							// check for consecutive shots from other players
							int numConsShots = history.consecutiveShots(shooters[i], friends.get(i));
							if (numConsShots == 1) {
								if (enemies.contains(shooters[i])) {
									weights[shooters[i]] += ENEMY_SHOOTS_FRIEND;
								}
								else if (!friends.contains(shooters[i])) {
									weights[shooters[i]] += NEUTRAL_SHOOTS_FRIEND;
								}
							}
							else if (numConsShots > 1) {
								if (enemies.contains(shooters[i])) {
									weights[shooters[i]] += ENEMY_SHOOTS_FRIEND_LOTS * numConsShots;
								}
								else if (!friends.contains(shooters[i])) {
									weights[shooters[i]] += NEUTRAL_SHOOTS_FRIEND_LOTS * numConsShots;
								}
							}
						}
					}
	...
	private int breakTie(int[] targets) {
		...
		for (int i = 0; i < targets.length; i++) {
			secondaryWeights[targets[i]] += history.shotsFiredOnPlayer(targets[i]);
			secondaryWeights[targets[i]] -= history.getRetaliationFactor(targets[i]);
			secondaryWeights[targets[i]] -= history.getReinforcementFactor(targets[i], alive);
		}
	...
	public int getBestTarget() {
		...
		if (candidates.length > 1) {
			//target = breakTie(candidates);
			target = candidates[gen.nextInt(candidates.length)];
		}
		else {
			target = candidates[0];
		}
